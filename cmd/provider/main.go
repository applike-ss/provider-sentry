...
import (
	...
	"github.com/justtrack/provider-sentry/apis"
	"github.com/justtrack/provider-sentry/apis/v1alpha1"
	"github.com/justtrack/provider-sentry/config"
	"github.com/justtrack/provider-sentry/internal/clients"
	"github.com/justtrack/provider-sentry/internal/controller"
	"github.com/justtrack/provider-sentry/internal/features"
)

func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Sentry").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-sentry"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-sentry",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add Sentry APIs to scheme")
	...
	kingpin.FatalIfError(controller.Setup(mgr, o), "Cannot setup Sentry controllers")
